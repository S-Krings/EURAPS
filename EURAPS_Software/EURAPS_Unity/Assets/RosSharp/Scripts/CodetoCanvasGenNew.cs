...
public class CodetoCanvasGenNew : MonoBehaviour
{
    ...
    public MovementController movementController; //Add movementController of robot for simulation here//TODO can be deleted when new simulation calling works
    public SimulationCommunicator simulationCommunicator;

    public List<SPEAREDCommand> commandTypes = new List<SPEAREDCommand>();

    // Start is called before the first frame update
    void Start()
    {
        RobotProgramStructure newrobotProgramStructure = new RobotProgramStructure(commandTypes);
    ...
    */
    ...
    public void drawCurrentCode()
    {
        ...
        foreach (RuntimeCommand statement in ProgramStore.GetInstance().getCommands())
        {
            //debug
    ...
    public void makeAndRun()
    {
        ...
        var ps = ProgramStore.GetInstance();
        RuntimeCommand cmd = ScriptableObject.CreateInstance<RuntimeCommand>();
        cmd.arguments = new List<object>();
        cmd.arguments.Add(new Vector2(60, 60));
        cmd.command = ScriptableObject.CreateInstance<SPEAREDCommand>();
        cmd.command.isLegoCommand = true;
        cmd.command.Lcommand = LegoCommand.MOVE;
        cmd.command.name = "Move";
        cmd.command.Signature = ScriptableObject.CreateInstance<SPEAREDCommandSignature1>();
        cmd.command.Signature.name = "Vector2d";
        (cmd.command.Signature as SPEAREDCommandSignature1).argumentType = SPEAREDCommandSignature1.ArgumentType.Vector2d;
        (cmd.command.Signature as SPEAREDCommandSignature1).argumentName = "target";
        ps.AddCommand(cmd);

        var executor = NXTExecutor.GetInstance();
        executor.send();
    ...
    public void simulateRun()
    {
        ...
        simulationCommunicator.simulate();
    ...
    public void simulation(Program program)
    {
        ...
        foreach (Statement statement in program.statements)
        {
            if (statement is MoveStatement1 moveStatement)
            {
                ...
                values.Add(this.fixRotation(moveStatement.target));
            }
            else if (statement is BruteMove bmoveStatement)
            {
                //float x = bmoveStatement.target.x;
                ...
                //values.Add(new Vector3(x, 0, z));
                values.Add(this.fixRotation(bmoveStatement.target));
