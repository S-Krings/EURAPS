...
ToggleSuction(False);
*/
    public RobotProgramStructure(List<SPEAREDCommand> cmds)
    {
        ...
        else
        {
            ...
            statements = newMovementCommand(new Vector3(0.6f, 0f, 0f), new MoveToJ1(), statements);
            ClawUp clawUp2 = new ClawUp();
            clawUp2.isClawUp = false;
            statements.Add(clawUp2);
            statements = newMovementCommand(new Vector3(1.1f, 0f, 0f), new MoveToJ1(), statements);
            //Generate a ClawUp command below
            clawUp = new ClawUp();
            //Debug.Log("ClawUp is being tried to create!");
            clawUp.isClawUp = true;
            statements.Add(clawUp);
            statements = newMovementCommand(new Vector3(0.9f, 0f, 0.4f), new MoveToJ1(), statements);

            RuntimeCommand cmd1 = ScriptableObject.CreateInstance<RuntimeCommand>();
            // Claw up
            cmd1.command = cmds[2];
            ProgramStore.GetInstance().AddCommand(cmd1);
            RuntimeCommand cmd2 = ScriptableObject.CreateInstance<RuntimeCommand>();
            // Move
            cmd2.command = cmds[0];
            cmd2.arguments = new List<object>();
            cmd2.arguments.Add(0.6f);
            cmd2.arguments.Add(0.6f);
            ProgramStore.GetInstance().AddCommand(cmd2);
            RuntimeCommand cmd3 = ScriptableObject.CreateInstance<RuntimeCommand>();
            // Claw down
            cmd3.command = cmds[3];
            ProgramStore.GetInstance().AddCommand(cmd3);
            RuntimeCommand cmd4 = ScriptableObject.CreateInstance<RuntimeCommand>();
            // Move
            cmd4.command = cmds[0];
            cmd4.arguments = new List<object>();
            cmd4.arguments.Add(1.1f);
            cmd4.arguments.Add(0);
            ProgramStore.GetInstance().AddCommand(cmd4);
            RuntimeCommand cmd5 = ScriptableObject.CreateInstance<RuntimeCommand>();
            // Claw up
            cmd5.command = cmds[2];
            ProgramStore.GetInstance().AddCommand(cmd5);
            RuntimeCommand cmd6 = ScriptableObject.CreateInstance<RuntimeCommand>();
            // Move
            cmd6.command = cmds[0];
            cmd6.arguments = new List<object>();
            cmd6.arguments.Add(0.9f);
            cmd6.arguments.Add(0.4f);
            ProgramStore.GetInstance().AddCommand(cmd6);
        }
        /*
         * not possible
         *  0 24 0
