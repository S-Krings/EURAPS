...
public class MovementController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (autostartTestrun)
        {
            statesList = new List<states> { states.RELEASE, states.MOVE, states.GRAB, states.MOVE, states.RELEASE };
            positionsList = new List<Vector3> { testPosition, new Vector3(0, 0, 12f), testPosition, new Vector3(8f, 0, 18f), testPosition};
    ...
    public void simulateProgram(List<states> states, List<Vector3> goals)
    {
        ...
        //foreach(Vector3 v in positionsList)
        {
            ...
            v.Set((float)Math.Round(positionsList[i].x, 2), robotHeight, (float)Math.Round(positionsList[i].z,2));
            positionsList[i] = v;
        }
        Debug.Log("ROBOT Commands:");
        for(int i = 0; i<states.Count; i++)
        {
            Debug.Log("#####"+states[i] + " " + goals[i].ToString("F3"));
        }
        nextState();
    }
    ...
    public void grabUpdate()
    {
        ...
        else
        {
            ...
            {
                claw.setState(ClawControl.clawstates.HOLD);
                claw.playSound();
                nextState();
            }
        }
